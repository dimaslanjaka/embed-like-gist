...
// usage
// <script src="embed-v2.js?target=https%3A%2F%2Fgithub.com%2Fdimaslanjaka%2Fstatic-blog-generator%2Fblob%2Fe8ef351552d57c5e28e016e39e78fef139a8e7b2%2F.github%2Fworkflows%2Fbuild-beta.yml%23L152-L158&style=github&type=code&showBorder=on&showLineNumbers=on&showFileMeta=on&showFullPath=on&showCopy=on"></script>
...
<div id="${containerId}" class="emgithub-container">
  ...
  <div class="emgithub-file emgithub-file-${isDarkStyle ? 'dark' : 'light'}"
    ...
    ${
      ...
      <a target="_blank" href="${fileURL}">${decodeURIComponent(
            showFullPath ? filePath : pathSplit[pathSplit.length - 1]
          )}</a>
...
`
  ...
  const loadHLJS =
    typeof hljs != 'undefined' && typeof hljs.highlightElement != 'undefined' ? Promise.resolve() : loadScript(HLJSURL);
  ...
  const loadHLJSNum = loadHLJS.then(() =>
    typeof hljs.lineNumbersBlock != 'undefined' ? Promise.resolve() : loadScript(HLJSNumURL)
  );
  ...
  if (type === 'markdown' || type === 'ipynb') {
    const loadMarked =
      typeof marked != 'undefined'
        ? Promise.resolve()
        : loadScript('https://cdn.jsdelivr.net/npm/marked@4.0.18/marked.min.js');
    const loadMarkdownStyle = fetch(
      'https://cdn.jsdelivr.net/gh/sindresorhus/github-markdown-css@5.1.0/github-markdown-light.min.css'
    )
      .then((response) => response.text())
      .then((text) => {
        insertStyle(text);
    ...
    promises.push(loadMarked, loadMarkdownStyle, loadKatexStyle, loadHLJSStyle);

    if (type === 'ipynb') {
      const loadAnsiUp =
        typeof AnsiUp != 'undefined'
          ? Promise.resolve()
          : loadScript('https://cdn.jsdelivr.net/gh/drudru/ansi_up@4.0.4/ansi_up.min.js');
      const loadDOMPurify =
        typeof DOMPurify != 'undefined'
          ? Promise.resolve()
          : loadScript('https://cdn.jsdelivr.net/npm/dompurify@2.3.10/dist/purify.min.js');
      const loadNotebookjs = Promise.all([loadMarked, loadAnsiUp, loadDOMPurify])
        .then(() =>
          typeof nb != 'undefined'
            ? Promise.resolve()
            : loadScript('https://cdn.jsdelivr.net/gh/jsvine/notebookjs@0.6.7/notebook.min.js')
        )
        .then(() => {
    ...
    if (type === 'markdown' || type === 'ipynb') {
      ...
      const loadKatex =
        typeof katex != 'undefined'
          ? Promise.resolve()
          : loadScript('https://cdn.jsdelivr.net/npm/katex@0.16.0/dist/katex.min.js');
      const loadKatexAutoRender = loadKatex.then(() =>
        typeof renderMathInElement != 'undefined'
          ? Promise.resolve()
          : loadScript('https://cdn.jsdelivr.net/npm/katex@0.16.0/dist/contrib/auto-render.min.js')
      );
